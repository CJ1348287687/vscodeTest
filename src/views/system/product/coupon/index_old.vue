  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="couponList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="类型" prop="couponType" width="100" align="center">
        <template slot-scope="{ row }">
          ...
          <span v-if=" row.couponType==3">赠送</span>
        </template>
      </el-table-column>
      <el-table-column label="使用类型" prop="couponUseType" width="120" align="center">
        <template slot-scope="{ row }">
          <span v-if=" row.couponUseType==1">代金券</span>
          <span v-if=" row.couponUseType==2">折扣券</span>
          <span v-if=" row.couponUseType==3">满减券</span>
          <span v-if=" row.couponUseType==4">兑换券</span>
          <span v-if=" row.couponUseType==5">品类券</span>
          <span v-if=" row.couponUseType==6">商品券</span>
        </template>
      </el-table-column>
      <el-table-column label="使用门槛" prop="minPrice" width="120" align="center">
        <template slot-scope="{ row }">
          <span>{{ row.minPrice }}元</span>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="850px" append-to-body :close-on-click-modal="false" :show-close="false" :close-on-press-escape="false">
      <el-form ref="couponForm" :model="couponForm" :rules="couponRules" label-width="150px">
        ...
        <el-form-item label="优惠劵背景图" prop="couponBgId">
          <el-row>
            <el-col v-if="couponBackground.url != ''">
              <img
                :src="couponBackground.url"
                style="width: 148px;height: 148px ;border:1px solid"
                @click="backgroundPreview(couponBackground.url)"
              />
...
<!--            <el-radio :label="3">赠送券</el-radio>-->
        ...
        <el-form-item label="使用类型" prop="couponUseType">
          <el-radio-group v-model="couponForm.couponUseType">
            <el-radio :label="1">代金券</el-radio>
            <el-radio :label="2">折扣券</el-radio>
            <el-radio :label="3">满减券</el-radio>
            <el-radio :label="4" disabled>兑换券</el-radio>
            <el-radio :label="5" disabled>品类券</el-radio>
            <el-radio :label="6" disabled>商品券</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="" prop="money" v-if="couponForm.couponUseType == 1">
          <span slot="label">
            <el-tooltip content="优惠券面值,最小值为1,最大值为999999" placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip>
            优惠券面值
          </span>
          <el-input-number v-model="couponForm.money" :min="0" :max="999999" :precision="2" :controls="false" label=""></el-input-number>
          <span>   元</span>
        </el-form-item>
        <el-form-item label="" prop="discount" v-if="couponForm.couponUseType == 2">
          <span slot="label">
            <el-tooltip content="优惠券折扣,最小值为0.01,最大值为0.99,如0.85为85折" placement="top">
              <i class="el-icon-question"></i>
            </el-tooltip>
            折扣
          </span>
          <el-input-number v-model="couponForm.discount" :min="0.01" :max="0.99" :precision="2" :controls="false"
                           label=""
          ></el-input-number>
          <span>   折</span>
        </el-form-item>
        <el-row v-if="couponForm.couponUseType == 3">
          <el-col>
            <el-form-item label="" prop="max">
              <span slot="label">
                <el-tooltip content="满金额需大于减金额" placement="top">
                  <i class="el-icon-question"></i>
                </el-tooltip>
                满金额
              </span>
              <el-input-number v-model="couponForm.moMax" :min="0" :max="999999" :precision="2" :controls="false"
                               label=""
              ></el-input-number>
              <span>   元</span>
            </el-form-item>
          ...
          <el-col>
            <el-form-item label="" prop="out">
              <span slot="label">
                <el-tooltip content="减金额需小于满金额" placement="top">
                  <i class="el-icon-question"></i>
                </el-tooltip>
                减金额
              </span>
              <el-input-number v-model="couponForm.moOut" :min="0" :max="999999" :precision="2" :controls="false"
                               label=""
              ></el-input-number>
              <span>   元</span>
            </el-form-item>
        ...
        <!--          <el-radio-group v-model="couponFormShow.threshold">-->
        ...
        <el-form-item label="" prop="minPrice">
          <span slot="label">
                <el-tooltip content="如没有最低消费默认为0" placement="top">
                  <i class="el-icon-question"></i>
                </el-tooltip>
                优惠券最低消费
              </span>
          <el-input-number v-model="couponForm.minPrice" :min="0" :precision="2" :controls="false"></el-input-number>
          <span>   元</span>
        </el-form-item>
        ...
        <el-form-item label="排序" prop="sort">
          <el-input-number v-model="couponForm.sort" :min="0" :controls="false" label="排序"></el-input-number>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // selectUseType(type) {
    //   switch (type) {
    //     case 1: //代金券 优惠券面值，最低消费
    //       this.couponFormShow.money = true
    //       this.couponFormShow.discount = false
    //       this.couponFormShow.maxOut = false
    //       break
    //     case 2:// 折扣券
    //       this.couponFormShow.money = false
    //       this.couponFormShow.discount = true
    //       this.couponFormShow.maxOut = false
    //       break
    //     case 3:// 满减券
    //       this.couponFormShow.money = false
    //       this.couponFormShow.discount = false
    //       this.couponFormShow.maxOut = true
    //       break
    //     case 4://兑换券
    //       this.couponFormShow.money = false
    //       this.couponFormShow.discount = false
    ...
    reset() {
      // this.couponFormShow.money = false
      // this.couponFormShow.discount = false
      // this.couponFormShow.threshold = false
      // this.couponFormShow.maxOut = false

      this.couponForm.couponId = undefined
      this.couponForm.couponName = ''
      this.couponForm.couponType = 1
      this.couponForm.couponUseType = 1
      this.couponForm.money = 0
      this.couponForm.discount = 0.99
      this.couponForm.max = 0
      this.couponForm.out = 0
      this.couponForm.minPrice = 0
      ...
      this.couponForm.couponBgId = undefined
    ...
    handleUpdate(row) {
      ...
      couponInfo(params).then(response => {
        ...
        this.couponForm.couponUseType = Number(coupon.couponUseType)
        if(coupon.couponBgFile){
            this.couponForm.couponBgId = coupon.couponBgFile.fileId
            this.couponBackground.url = coupon.couponBgFile.fileVirtualPath
        }
        if (coupon.couponUseType == 1) {
          this.couponForm.money = coupon.money
          // this.couponFormShow.money = true
        } else if (coupon.couponUseType == 2) {
          this.couponForm.discount = coupon.discount
          // this.couponFormShow.discount = true
        } else if (coupon.couponUseType == 3) {
          this.couponForm.moMax = coupon.moMax
          this.couponForm.moOut = coupon.moOut
          // this.couponFormShow.maxOut = true
        } else if (coupon.couponUseType == 4) {
        } else if (coupon.couponUseType == 5) {
        } else if (coupon.couponUseType == 6) {
        }
        // 最低消费
        // if (coupon.minPrice > 0) {
        this.couponForm.minPrice = coupon.minPrice
        //   this.couponFormShow.threshold = true
        // } else {
        //   this.couponFormShow.threshold = false
        // }
        //领取限时
    ...
    couponSubmit() {
      ...
      // if (!this.couponFormShow.threshold) {
      //   // 如果选择的无门槛最小金额置为0 防止用户先点了有门槛再点了无门槛
      //   this.couponForm.minPrice = 0
      // }
    ...
    uploadTypeOpen() {
      ...
      inputFile.addEventListener('change', function() {
        ...
        couponUpload(formData).then(response => {
          _this.couponBackground.url = response.data.fileVirtualPath
          _this.couponForm.couponBgId = response.data.fileId
