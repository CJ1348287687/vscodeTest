      ...
      <el-table-column label="" type="expand" width="25">
        <template slot-scope="scope">
          <el-table
            ...
            @sort-change="
              (column, prop, order) =>
                handleSortChangeChannel(column, prop, order, scope.row)
            "
            @expand-change="expandChangeChannel"
            :row-style="channelColor"
          >
            <!-- 发展人列表 -->
            <el-table-column label="" type="expand" width="25">
              <template slot-scope="scope">
                <el-table
                  ...
                  "
                  :cell-style="{ padding: '3px' }"
                  :data="scope.row.shareList"
                  v-loading="loadingShare"
                  @sort-change="
                    (column, prop, order) =>
                      handleSortChangeShare(column, prop, order, scope.row)
                  "
                >
                  <!-- 二次分发手机号列表 -->
                  <!-- @expand-change="expandChangeShare"
                    :row-style="shareColor" -->
                  <!-- <el-table-column label="" type="expand" width="25">
                    <template slot-scope="scope">
                      <el-table :header-cell-style="{background:'#DDEBF7',color:'#333',padding: '3px'}"
                                style="margin-left:45px;box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1);width: auto;"
                                :cell-style="{padding: '3px'}"
                                :data="scope.row.sharePhoneList"
                                v-loading="loadingSharePhone"
                                @sort-change="(column, prop, order) => handleSortChangeSharePhone(column, prop, order,scope.row)"
                      >
                        ...
                        <el-table-column label="手机号码" prop="sts.phone" width="200" align="center" sortable="custom" :sort-orders="['descending', 'ascending']">
                          <template #default="scop">
                            {{ scop.row.sharePhone }}
                          </template>
                        </el-table-column>
                        <el-table-column label="浏览量" align="center" prop="pvCount" width="100">
                          <template #default="scope">
                              {{ scope.row.pvCount==null ? 0 : scope.row.pvCount}}
                          </template>
                        </el-table-column>
                        <el-table-column label="浏览人数" align="center" prop="uvCount" width="100">
                          <template #default="scope">
                        ...
                        <el-table-column label="留资量" align="center" prop="formRecordCount" width="100">
                          <template #default="scope">
                            <span v-if="scope.row.formCount!=0" >
                              <span v-if="scope.row.formRecordCount==0 || scope.row.formRecordCount==null">0</span>
                              <el-button v-if="scope.row.formRecordCount!=0 && scope.row.formRecordCount !=null"
                                size="mini" type="text"
                                @click="handleTargetList(scope.row,'form','sharePhone')" >
                                {{ scope.row.formRecordCount }}</el-button>
                            </span>
                          </template>
                        </el-table-column>
                        <el-table-column label="签到量" align="center" prop="signRecordCount" width="100">
                          <template #default="scope">
                            <span v-if="scope.row.signCount!=0">
                              <span v-if="scope.row.signRecordCount==0 || scope.row.signRecordCount==null">0</span>
                              <el-button v-if="scope.row.signRecordCount!=0 && scope.row.signRecordCount !=null"
                                size="mini" type="text"
                                @click="handleTargetList(scope.row,'sign','sharePhone')" >
                                {{ scope.row.signRecordCount }}</el-button>
                            </span>
                          </template>
                        </el-table-column>
                        <el-table-column label="领券量" align="center" prop="couponRecordCount" width="100">
                          <template #default="scope">
                            <span v-if="scope.row.couponCount!=0">
                              {{ scope.row.couponRecordCount==null ? 0 : scope.row.couponRecordCount}}
                            </span>
                          </template>
                        </el-table-column>
                      </el-table>
                  ...
                  >
                    <template #default="scope">
                      ...
                      >
                        {{
                          getValue(scope.row.countList, item.templateType)
                        }}</el-button
                      >
                    </template>
                  </el-table-column>
            ...
            <el-table-column label="渠道序号" align="center" width="100">
              <template slot-scope="scop" width="50">
                {{ scop.$index + 1 }}
              </template>
            </el-table-column>
            <el-table-column
              label="渠道所属组织"
              prop="channelorg.organizationName"
              align="center"
              width="150"
              sortable="custom"
              :sort-orders="['descending', 'ascending']"
            >
              <template #default="scop">
                {{ scop.row.channelOrganizationName }}
              </template>
            </el-table-column>
            <el-table-column
              label="渠道类型"
            ...
            >
              <template #default="scope">
                <el-button
                  ...
                  type="text"
                  @click="
                    handleTargetList(scope.row, item.templateType, 'channel')
                  "
                >
                  {{
                    getValue(scope.row.countList, item.templateType)
      ...
      >
        <template #default="scope">
          <el-button
            size="mini"
            type="text"
            @click="handleTargetList(scope.row, item.templateType, 'tweet')"
          >
        ...
        >
          <template slot-scope="scope">
            <el-button
              ...
              v-hasPermi="['system:manage:customerListExport']"
    ...
    >
      ...
      >
        ...
        <el-form-item
          label="渠道类型"
          prop="channelType"
          v-if="customerType == 'tweet'"
        >
          <el-select
            v-model="queryParamsCustomer.channelType"
            placeholder="请选择渠道类型"
            clearable
          >
            <el-option
        ...
        >
          <el-input
            v-model="queryParamsCustomer.channelName"
            placeholder="请输入渠道名称"
            clearable
            @keyup.enter.native="handleQueryCustomer"
          />
        ...
        >
          <el-input
            v-model="queryParamsCustomer.channelCode"
            placeholder="请输入渠道编码"
            clearable
            @keyup.enter.native="handleQueryCustomer"
          />
...
export default {
  ...
  methods: {
    ...
    expandChange(row) {
      console.log(
        "===============",
        encodeURIComponent(window.btoa(18900001234))
      );
      if (this.clickTweetIds.includes(row.tweetId)) {
        this.clickTweetIds.splice(
          this.clickTweetIds.findIndex((item) => item === row.tweetId),
          1
        );
      } else {
    ...
    expandChangeChannel(row) {
      if (this.clickChannelIds.includes(row.channelId)) {
        this.clickChannelIds.splice(
          this.clickChannelIds.findIndex((item) => item === row.channelId),
          1
        );
      } else {
        this.clickChannelIds.push(row.channelId);
      }
      //查询数据
    ...
    expandChangeShare(row) {
      if (this.clickShareIds.includes(row.shareUserId)) {
        this.clickShareIds.splice(
          this.clickShareIds.findIndex((item) => item === row.shareUserId),
          1
        );
      } else {
        this.clickShareIds.push(row.shareUserId);
      }
      //查询数据
    ...
    getList() {
      ...
      tweetReport(this.addDateRange(this.queryParams, this.dateRange)).then(
        (response) => {
    ...
    handleSortChangeChannel(column, prop, order, parentRow) {
      ...
      channelReport({
        ...
        parentRow.channelList = response.rows;
        this.loadingChannel = false;
      });
    },
    handleSortChangeShare(column, prop, order, parentRow) {
      this.loadingShare = true;
      shareReport({
        pageNum: 1,
        pageSize: 1000,
        ...
        parentRow.shareList = response.rows;
        this.loadingShare = false;
      });
    },
    ...
    handleTargetList(row, templateType, type) {
      ...
      this.queryParamsTarget.searchType = "manage";
      this.queryParamsTarget.organizationId =
        sessionStorage.getItem("organizationId");
      if (type == "tweet") {
        this.queryParamsTarget.tweetId = row.tweetId;
        this.queryParamsTarget.channelId = null;
        this.queryParamsTarget.shareUserId = null;
    ...
    /** 推文下表单/签到列表 重置按钮操作 */
    resetQueryTarget() {
      this.queryParamsTarget.productName = "";
